...
function makeConfirmedEnvelope(conflicts = []) {
  return {
    ...
    claim: 'A confirmed claim must not silently ignore evidence marked contradictory.',
    scope: {},
    status: 'Confirmed',
    evidence: [
...
test('Confirmed claims reject contradictory evidence with no explicit disposition', () => {
  ...
  assert.ok(
    errors.includes('Confirmed claim has undispositioned contradictory evidence: contradict-1'),
  ...
  const envelope = makeConfirmedEnvelope([
    {
      ...
      state: 'resolved',
      resolution_basis: 'The supporting source supersedes the contradictory source at the exact scope.',
    },
  ]);

...
Current plan note (2026-09-14): OpenAI's current plan-specific Help Center guidance explicitly says **Pro** users can connect MCPs with read/fetch permissions in developer mode, while **Full MCP** is currently limited to Business and Enterprise/Edu. The current article does not grant ChatGPT Plus an equivalent custom MCP developer-mode entitlement. For this repository's exact compatibility signal, therefore treat Plus private custom MCP / Secure MCP Tunnel parity as **closed / not currently supported by published plan guidance**. This is not proof that no Plus account can ever expose an experiment; reopen the gate only on unambiguous Plus-specific first-party guidance or bounded authenticated evidence from the target account. Tunnel authorization and ChatGPT plan entitlement remain separate checks.

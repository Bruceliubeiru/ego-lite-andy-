...
export function selectNextResearchAction({ state, candidates = [] }) {
  ...
  const materialReopenCoverageKeys = new Set(deriveMaterialReopenCoverageKeys({ evidence: state?.material_reopen_evidence }));
  const eligible = candidates.filter((action) => {
    ...
    const coverageKey = canonicalCoverageKey(action.coverage_key);
    if (coverageKey && completedCoverageKeys.has(coverageKey) && !materialReopenCoverageKeys.has(coverageKey)) return false;
    return true;
  });
  ...
  return { mode: 'execute', action_id: ranked[0].action.id, reason: 'highest-decision-value-safe-read' };
}

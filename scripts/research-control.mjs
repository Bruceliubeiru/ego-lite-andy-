...
const EXPECTED_DELTAS = new Set([
  'resolve_scope', 'resolve_conflict', 'establish_provenance', 'add_independent_lineage',
  'test_causal_hypothesis', 'change_decision',
]);
...
function compareRank(a, b) { const ar = rank(a); const br = rank(b); for (let i = 0; i < ar.length; i += 1) if (ar[i] !== br[i]) return br[i] - ar[i]; return 0; }
...
function dependencyTarget(receipt, dependencies = []) {
  const source = canonicalCoverageKey(receipt?.action?.coverage_key);
  const requestedTarget = canonicalCoverageKey(receipt?.action?.reopen_target_coverage_key);
  const beforeRevision = receipt?.before?.revision;
  if (!source || !requestedTarget || !isNonNegativeInteger(beforeRevision)) return null;
  const matched = (Array.isArray(dependencies) ? dependencies : []).some((dependency) =>
    canonicalCoverageKey(dependency?.source_coverage_key) === source &&
    canonicalCoverageKey(dependency?.target_coverage_key) === requestedTarget &&
    dependency?.revision === beforeRevision
  );
  return matched ? requestedTarget : null;
}

function materialTransition(receipt, dependencies = []) {
  const producingCoverageKey = canonicalCoverageKey(receipt?.action?.coverage_key);
  const receiptCoverageKey = canonicalCoverageKey(receipt?.coverage_key);
  const beforeRevision = receipt?.before?.revision;
  const afterRevision = receipt?.after?.revision;
  if (!producingCoverageKey || receiptCoverageKey !== producingCoverageKey || !isNonNegativeInteger(beforeRevision) || !isNonNegativeInteger(afterRevision) || afterRevision !== beforeRevision + 1) return null;
  const requestedTarget = canonicalCoverageKey(receipt?.action?.reopen_target_coverage_key);
  const targetCoverageKey = requestedTarget ? dependencyTarget(receipt, dependencies) : producingCoverageKey;
  if (!targetCoverageKey) return null;
  return {
    coverage_key: targetCoverageKey,
    transition_key: `${producingCoverageKey}${targetCoverageKey === producingCoverageKey ? '' : `=>${targetCoverageKey}`}@${beforeRevision}->${afterRevision}`,
...
export function evaluateDecisionDelta({ action, before, after, observation } = {}) {
  ...
  const changed = {
    ...
    establish_provenance: typeof before?.provenance_established === 'boolean' && typeof after?.provenance_established === 'boolean' && !before.provenance_established && after.provenance_established,
...
export function deriveMaterialReopenAuthorizations({ evidence = [], consumed = [], dependencies = [] } = {}) {
  const consumedKeys = new Set((Array.isArray(consumed) ? consumed : []).filter((value) => typeof value === 'string').map((value) => value.trim()).filter(Boolean));
  const authorizations = new Map();
  for (const receipt of Array.isArray(evidence) ? evidence : []) {
    const transition = materialTransition(receipt, dependencies);
    if (!transition || receipt?.material_state_observed !== true) continue;
    ...
    const current = authorizations.get(transition.coverage_key);
    if (!current || transition.after_revision > current.after_revision) authorizations.set(transition.coverage_key, transition);
  }
  return [...authorizations.values()].filter((authorization) => !consumedKeys.has(authorization.transition_key));
}

export function deriveMaterialReopenCoverageKeys({ evidence = [], consumed = [], dependencies = [] } = {}) { return deriveMaterialReopenAuthorizations({ evidence, consumed, dependencies }).map((authorization) => authorization.coverage_key); }
...
export function updateResearchRunState({ state = {}, selection, action, outcome } = {}) {
  ...
  const consumed = new Set((Array.isArray(state?.consumed_reopen_transition_keys) ? state.consumed_reopen_transition_keys : []).filter((value) => typeof value === 'string').map((value) => value.trim()).filter(Boolean));
  const actionCoverageKey = canonicalCoverageKey(action?.coverage_key);
  const currentAuthorization = actionCoverageKey ? new Map(deriveMaterialReopenAuthorizations({ evidence: state?.material_reopen_evidence, consumed: state?.consumed_reopen_transition_keys, dependencies: state?.reopen_dependencies }).map((authorization) => [authorization.coverage_key, authorization])).get(actionCoverageKey) : null;
...
export function selectNextResearchAction({ state, candidates = [] }) {
  ...
  const materialReopenAuthorizations = new Map(deriveMaterialReopenAuthorizations({ evidence: state?.material_reopen_evidence, consumed: state?.consumed_reopen_transition_keys, dependencies: state?.reopen_dependencies }).map((authorization) => [authorization.coverage_key, authorization]));
  const eligible = candidates.filter((action) => {
    if (!action || typeof action.id !== 'string' || !action.id.trim() || !hasValidRankMetadata(action)) return false;
    if (action.decision_relevance === 'none' || action.scope_fit === 'wrong' || action.blocked_on_observation === true || action.risk !== 'read_only') return false;
  ...
  const ranked = eligible.map((action, index) => ({ action, index })); ranked.sort((left, right) => compareRank(left.action, right.action) || left.index - right.index);
